...
class pattern(object):
    ...
    def generateRandomPattern(self, rng):
        ...
        for i in range(self.firing):
            unordered_events.append((int(randomNeurons[i]), round(randomTimes[i], 1)))
    ...
    def compareSpikeTrains(self, observedSpikes, tolerance = 2.5, display=False):
        ...
        for pattSpike in self.events:
            ...
            for spike in myObservedSpikes:
                ...
                #print "NID: %d  time: %f " % (neuronID, timeIndex)
                if pattNeuronID == neuronID:
                    diff = timeIndex - pattTimeIndex
                    abs_diff = abs(diff)
                    diff2 = self.cycleTime - diff
                    if abs(diff) > abs(diff2):
                       diff = diff2
                    totalSpikes += 1
                    totalDev += diff
                    totalDevSq += abs_diff*abs_diff
                    ...
                    if display:
                        print "Pattern time: ", pattTimeIndex, ",  observed: ", timeIndex, ", diff:", diff
        ...
        if totalSpikes == 0:
            return -1, -1, -1, -1, -1
        meanDev = totalDev / (1.0 * totalSpikes)
        stdDev = math.sqrt(totalDevSq)/ ( 1.0* totalSpikes)
    
        return truePositives, falsePositives, falseNegatives, meanDev, stdDev
...
class spikeStream(object):
    ...
    def buildStream(self, numSources=None, patterns=None, interPatternGap=10, rng=None, offset=0.0, order=None, noise=None, printTimes = False):
        ...
        timePtr = 0
        jitterDistribution = RandomDistribution('normal', parameters = [0.0, patterns[0].jitterSD], rng = rng)
        for entry in order:
            if entry < 0:
                # Add blank entry:
                ...
                for element in pattern.events:
                    ...
                    #print "NID: %d, time: %d" %(index,timestamp)
                    timestamp += offset
